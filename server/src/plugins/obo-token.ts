...
const getOboToken: GetOboToken = async (appName, req, reply) => {
  ...
  try {
    ...
    log.debug({ msg: `Requesting OBO token for "${NAIS_CLUSTER_NAME}:klage:${appName}".`, ...logParams });
    ...
    if (!oboAccessToken.ok) {
      log.warn({ msg: `Failed to get OBO token for audience: ${appName}.`, ...logParams });

      return undefined;
    }

    log.debug({ msg: `OBO token for "${NAIS_CLUSTER_NAME}:klage:${appName}" received.`, ...logParams });

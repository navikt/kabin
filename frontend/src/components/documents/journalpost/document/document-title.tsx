import { DocumentWarnings } from '@app/components/documents/journalpost/document/document-warnings';
import { EditTitle } from '@app/components/documents/journalpost/document/edit-document-title';
import {
...
export const DocumentTitle = ({ dokument }: Props) => {
  ...
  const isActive = useMemo(
    () =>
      viewed !== null &&
      !isViewedUploadedDokument(viewed) &&
      viewed.dokumentInfoId === dokumentInfoId &&
      viewed.journalpostId === journalpostId,
  ...
  if (!editMode) {
    return (
      <Tooltip content={tittel} delay={500}>
        <HStack align="center" gap="space-4" height="2rem" overflow="hidden" wrap={false}>
          <div
            data-testid="document-title"
            className={`grow select-text truncate ${isActive ? 'font-bold' : 'font-normal'}`}
          >
            {tittel}
          </div>

  ...
  const updateState = useCallback(
    (newState: State<P, S>) => {
      ...
      if (isSvarbrevUpdate(update) && isSvarbrevState(state)) {
        ...
        if (overstyringer !== undefined) {
          ...
          if (fullmektig !== undefined && fullmektig !== previousFullmektig) {
            ...
            if (update.svarbrev !== undefined) {
              ...
              update.svarbrev = { ...structuredClone(state.svarbrev), fullmektigFritekst };
            }
          }
        }

        const updatedState = { ...structuredClone(state), ...update };

        if (overstyringer === undefined || state.mulighet === null) {
        ...
        const incomingReceivers = update.svarbrev?.receivers ?? structuredClone(previousReceivers);

        const updatedReceivers = incomingReceivers.filter((r) => !removedSuggestions.some((s) => s.id === r.part.id));

        const fullmektigShouldStillBeSelected =
          previousFullmektig !== null && incomingReceivers.some((r) => r.part.id === previousFullmektig.id);

        const klagerShouldStillBeSelected =
          previousKlager !== null && incomingReceivers.some((r) => r.part.id === previousKlager.id);

        if (
          fullmektigShouldStillBeSelected &&
      ...
      return setState({ ...structuredClone(state), ...update });
    },
    [errors, state],
  );
